...
"	margin:auto;	}</style>");
   ...
   out.println("<title>Servlet ViewQueriesServlet</title>");   
   out.println("</head>");
   out.println("<body>");
   //code to display queries
   ...
   try
   {
    Class.forName("com.mysql.jdbc.Driver");
      ...
      PreparedStatement ps=conn.prepareStatement
        ("Select qid,query From queriestbl Where techid=? order by qid");

         int techid=Integer.parseInt
                 ( session.getAttribute("exptechid").toString());         
        
      ps.setInt(1, techid);
      ResultSet rs=ps.executeQuery();
    out.println("<div class='container'>");
      out.println("<center><table border='2'><tr><th>Query</th><th>Action</th></tr></center>");
      while(rs.next())
      {
       out.println("<div class='well'>");
       
       out.println("<form method='get' action='PostAnsServlet'>");
       out.println("<tr><td>"+rs.getString("query")+"</td>");      
       
       out.println("<tr><td><input  type='hidden' name='tbq' value="
               +rs.getString("qid").toString());
       
       out.println("</input></td>");
      out.println
        ("<td><input type='submit' name='btnans' value='Answer' ></input></td></tr>");
       out.println("</form>");
       out.println("</div>");
      }
         out.println("</table>");
